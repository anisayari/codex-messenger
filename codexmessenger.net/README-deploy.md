...
- `downloads/CodexMessenger.exe.sha256`: checksum for the Windows installer generated by the latest successful release deployment.
- `downloads/CodexMessenger-mac-arm64.dmg.sha256`: checksum for the macOS installer generated by the latest successful release deployment.
- `downloads/.gitkeep`: keeps the download directory in Git.
- `nginx.codexmessenger.net.conf`: nginx vhost used on the VPS.

The Windows and macOS installers are not committed to Git. The GitHub Actions deployment resolves the latest release `.exe` and `.dmg`, patches the download cache parameters in `index.html`, sends the static bundle to the VPS, then lets the VPS download the release assets directly as `downloads/CodexMessenger.exe` and `downloads/CodexMessenger-mac-arm64.dmg` and write checksums.
...
- prefers a `*Setup*.exe` Windows asset, then falls back to the first `.exe` asset
- prefers an `arm64` macOS `.dmg` asset, then falls back to the first `.dmg` asset
- verifies SHA-256 digests when GitHub exposes them
...
4. Open the `.sha256` files under `https://codexmessenger.net/downloads/` if you need to verify deployed checksums.
...
sudo rsync -av --delete \
  ...
  --exclude 'downloads/CodexMessenger-mac-arm64.dmg' \
