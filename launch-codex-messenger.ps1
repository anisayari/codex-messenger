...
function Get-InstallCandidates {
  ...
  $dirs += Join-Path $scriptRoot "release\win-unpacked"
...
function Start-CodexMessenger {
  ...
  Start-Process -FilePath $npm -ArgumentList @("run", "electron:dev") -WorkingDirectory $scriptRoot -WindowStyle Hidden
}
...
function Show-Launcher {
  ...
  $pathText = if ($install.InstallDir) { $install.InstallDir } else { $scriptRoot }
